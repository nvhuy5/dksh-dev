...
def metadata_extract(self: ProcessorBase, data_input, schema_object, response_api, *args, **kwargs) -> StepOutput: # NOSONAR
    ...
    try:
        ...
        logger.exception(f"[metadata_extract] An error occurred: {e}", exc_info=True)
        error_msg = (
            "[metadata_extract] missing data_input from previous step"
            if data_input is None
            else f"[metadata_extract] An error occurred: {e}"
        )
        return StepOutput(
            data=schema_object.model_copy( # NOSONAR
                update={
                    "messages" : [error_msg]
                }
            ),
