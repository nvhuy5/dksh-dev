...
def template_data_mapping(self: ProcessorBase, data_input, schema_object, response_api, *args, **kwargs) -> StepOutput: # NOSONAR
    """
    ...
    try:
        ...
        # Validate expected headers from API vs actual DataFrame columns
        expected_headers = [
            m["fromHeader"]
        ...
        data_output.update(
            {
                ...
                "mappedHeaders": len(mapping_dict) + number,
                "unmappedHeaders": len(headers_sorted) - len(mapping_dict) - number,
            }
        )
        ...
        error_msg = (
            "[template_data_mapping] missing data_input from previous step"
            if data_input is None
            else f"[template_data_mapping] An error occurred: {e}"
        )
        return StepOutput(
