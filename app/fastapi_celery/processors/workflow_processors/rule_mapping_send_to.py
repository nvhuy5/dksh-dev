...
def send_to(self: ProcessorBase, data_input, schema_object, response_api, *args, **kwargs) -> StepOutput: # NOSONAR
    ...
    try:
        ...
        logger.exception(f"[send_to] An error occurred: {e}", exc_info=True)
        error_msg = (
            "[send_to] missing data_input from previous step"
            if data_input is None
            else f"[send_to] An error occurred: {e}"
        )
        return StepOutput(
