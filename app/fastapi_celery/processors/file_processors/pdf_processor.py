...
# === Set up logging ===
logger = log_helpers.get_logger("PDF Processor")

LINE_ENDS_WITH_COLON_PATTERN = re.compile(r".+[：:]\s*$")
KV_PATTERN = re.compile(r"([^：:\s]+)[：:]\s*")


class Pdf001Template: # pragma: no cover  # NOSONAR
    """
    ...
    def parse_file_to_json(self) -> PODataParsed:
        ...
        try:
            if self.file_record.get("source_type") == "local":
                doc = fitz.open(self.file_record.get("file_path"))
            else:
                self.file_record.get("object_buffer").seek(0)
                doc = fitz.open(
                    stream=self.file_record.get("object_buffer").read(),
                    filetype="pdf"
                )
            ...
            return build_success_response(
                self.file_record.get("file_path"), self.file_record.get("document_type"), self.po_number, items, metadata, self.file_record.get("file_size")
            )

        except Exception as e:
            return build_failed_response(
                self.file_record.get("file_path"),
                self.file_record.get("document_type"),
                self.file_record.get("file_size"),
                e,
            )


class Pdf002Template: # pragma: no cover  # NOSONAR
    """
    ...
    def parse_file_to_json(self) -> PODataParsed:
        ...
        try:
            ...
            if self.file_record.get("source_type") == "local":
                pdf_source = self.file_record.get("file_path")
            else:
                self.file_record.get("object_buffer").seek(0)
                pdf_source = self.file_record.get("object_buffer")
...
class Pdf004Template: # pragma: no cover  # NOSONAR
    """
...
class Pdf006Template: # pragma: no cover  # NOSONAR
    """
...
class Pdf007Template: # pragma: no cover  # NOSONAR
    """
...
class Pdf008Template: # pragma: no cover  # NOSONAR
    """
